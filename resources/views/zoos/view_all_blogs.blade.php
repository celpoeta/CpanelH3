@php
    ...
    $last_blog_user = App\Models\User::find($last_blog->created_by);
@endphp
@extends('layouts.pages.pages_master')
        ...
        <select class="btn btn-primary me-2 nice-select"
            ...
            @foreach ($languages as $language)
                <option class="" @if ($lang == $language) selected @endif
                    value="{{ route('see.all.blogs', $language) }}">{{ Str::upper($language) }}
                </option>
            @endforeach
        </select>
    </li>
                ...
                <div class="row">
                                    ...
                                    <div class="article-card">
                                        <div class="article-card-inner  d-flex align-items-center">
                                            <div class="card-image">
                                                <a href="{{ route('view.blog', $recent_blog->slug) }}" tabindex="0">
                                                    <img src="{{ isset($recent_blog->images) ? Storage::url($recent_blog->images) : asset('vendor/landing-page2/image/blog-card-img.png') }}"
                                                        alt="recent-post-image">
                                                </a>
                                            </div>
                                            <div class="card-content">
                                                <h6>
                                                    <a href="{{ route('view.blog', $recent_blog->slug) }}">{{ isset($recent_blog->title) ? $recent_blog->title : __('Prime-laravel') }}</a>
                                                </h6>
                                                <div class="date d-flex align-items-center">
                                                    ...
                                                    {{ App\Facades\UtilityFacades::date_time_format($recent_blog->created_at) }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="article-card">
                                    <div class="article-card-inner">
                                        <div class="article-card-image">
                                            <a href="{{ route('view.blog', $last_blog->slug) }}">
                                                <img src="{{ isset($last_blog->images) ? Storage::url($last_blog->images) : asset('vendor/landing-page2/image/blog-card-img.png') }}"
                                                    alt="blog-card-image">
                                            </a>
                                        </div>
                                            ...
                                            <div class="author-info d-flex align-items-center justify-content-between">
                                                <div class="author-name d-flex align-items-center">
                                                    <div class="author-img">
                                                        <img src="{{ Storage::url($last_blog_user->avatar) }}"
                                                            alt="client-image">
                                                    </div>
                                                    <span>{{ $last_blog_user->name }}</span>
                                                </div>
                                                <div class="date d-flex align-items-center">
                                                    ...
                                                    <span>{{ App\Facades\UtilityFacades::date_time_format($last_blog->created_at) }}</span>
                                                </div>
                                            </div>
                                            <h3>
                                                <a
                                                    href="{{ route('view.blog', $last_blog->slug) }}">{{ isset($last_blog->title) ? $last_blog->title : __('Benefits of Multi-Tenancy in Laravel Dashboard') }}</a>
                                            </h3>
                                            <p>{!! isset($last_blog->short_description)
                    ...
                    <div class="col-lg-8 col-md-7 col-12">
                        <div class="blog-right-side append-blog-data">
                            ...
                            @endforeach
                            {{ $all_blogs->links('blog.custom.pagination') }}
                        </div>
                    </div>
                </div>
            </div>
