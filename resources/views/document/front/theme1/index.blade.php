...
<head>
    ...
    <link rel="icon"
        href="{{ setting('favicon_logo') ? Storage::url('appLogo/app-favicon-logo.png') : asset('assets/images/app-favicon-logo.png') }}"
        type="image/png">
                ...
                <div class="main-content">
                    <section class="section">
                        <div class="section-header">
                            ...
                            @if (empty($changelog))
                                @if (isset($document_menu->title))
                                    <h1>{{ $document_menu->title }}</h1>
                                    <div class="section-header-breadcrumb">
                                        @if ($menus->id == $document_menu->parent_id)
                                            <div class="breadcrumb-item">
                                                <a
                                                    href="{{ route('documentmenu.menu', $menus->slug) }}">{{ $menus->title }}</a>
                                            </div>
                                        @endif
                                        <div class="breadcrumb-item">{{ $document_menu->title }}</div>
                                    </div>
                                @else
                                    @php
                                        $doc_menu = App\Models\DocumentMenu::where('document_id', $document->id)
                                            ->orderBy('position')
                                            ->first();
                                    @endphp
                                    <h1>{{ $doc_menu->title }}</h1>
                                    <div class="section-header-breadcrumb">
                                        <div class="breadcrumb-item">{{ $doc_menu->title }}</div>
                                    </div>
                                @endif
                            @else
                                <h1>{{ __('Change Log') }}</h1>
                                    ...
                                    <div class="card txt-crd">
                                        <div class="card-body">
                                            @if (empty($changelog))
                                                @if (isset($document_menu->title))
                                                    @php
                                                        $array = json_decode($document_menu->json);
                                                    @endphp
                                                    @if (isset($array))
                                                ...
                                                @else
                                                    @php
                                                        $doc_menu = App\Models\DocumentMenu::where('document_id', $document->id)
                                                            ->orderBy('position')
                                                            ->first();
                                                        $val = isset($doc_menu->json) ? $doc_menu->json : '';
                                            ...
                                            @else
                                                <section class="page-content">
                                                    <h2>Changelog</h2>
                                                    <p>See what's new added, changed, fixed, improved or
                                                        updated in the latest
                                                        versions. </p>
                                                    <div class="page-block-content">
                                                        @php
                                                            if ($change_log_jsons) {
                                                                usort($change_log_jsons, function ($a, $b) {
                                                                    return version_compare($b['version'], $a['version']);
                                                                });
                                                            }
                                                        @endphp
                                                        <hr class="divider">
                                                        @foreach ($change_log_jsons as $changeLog)
                                                            @php
